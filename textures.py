...
def perlin_noise_point(x, y,
                       vector_cell_size: tuple[float, float],
                       seed: int,
                       strength: float,
                       bias: float,
                       position_x: float,
...
def direct_perlin_noise(image: np.ndarray,
    ...
    if x >= image.shape[0] or y >= image.shape[1]:
        return
    vector_cell_size = image.shape[0] / vector_grid_resolution, image.shape[1] / vector_grid_resolution
    image[x, y] += perlin_noise_point(x, y, vector_cell_size, seed, strength, bias, position_x, position_y)
...
# @gpu_program(['(float64[:,:], float64, int64, int64, float64, float64, float64[:])'])
@gpu_program(default_shape_source=0)
def direct_height_map(image: np.ndarray,
                      base_height: float,
                      seed: int,
                      iterations: int,
                      contrast: float,
    ...
    for i in range(iterations):
        ...
        if layer == 1:
            bias = base_height / 2
        vector_grid_resolution = 2 ** layer / zoom
        vector_cell_size = image.shape[0] / vector_grid_resolution, image.shape[1] / vector_grid_resolution
        position_x, position_y = position
        strength = base_height / (2 ** (layer ** (1 / (contrast + 1))))
        image[x, y] += perlin_noise_point(x, y, vector_cell_size, seed, strength, bias, position_x, position_y)
...
def generate_height_map(image: np.ndarray,
    ...
    #                  position,
    #                  seed=seed)
    direct_height_map(image, base_height, seed, iterations, contrast, zoom, position, limit_threads=512+256+64)
